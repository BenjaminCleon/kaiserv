    ...
    def events(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONUP and event.button == 1 and self.can_thinking:
            #for grid in self.grid_to_draw:
                    ...
                    #self.carriere.controleur.clear(grid, self.carriere.dictionnaire_reverse_by_path[self.path])
        ...
        if self.is_progress:
            for grid in self.grid_to_draw:
                if grid[0] >= 0 and grid[1] >= 0 and len(self.carriere.informations_tiles) > grid[0] and len(
                        self.carriere.informations_tiles[grid[0]]) > grid[1]:
                    #is_construction_possible_on_this_grid = self.carriere.controleur.check_if_construction_possible_on_grid( grid)
                   # if is_construction_possible_on_this_grid:
                        #color = (0, 255, 0)
                    #else:
                        #color = (255, 0, 0)
                    iso_for_validation = [(
                                          x * self.carriere.zoom.multiplier + self.carriere.current_surface.get_width() / 2 + self.carriere.camera.scroll.x,
                                          y * self.carriere.zoom.multiplier + self.carriere.camera.scroll.y) for x, y in
                                          self.carriere.informations_tiles[grid[0]][grid[1]]["iso"]]
                    pygame.draw.polygon(self.carriere.controleur.screen, menu_settings.RED, iso_for_validation, 3)
        """